...
export async function activateInventoryItem(itemId: 'boost:recharge'): Promise<{ state: { coins: number; energy: number; maximumEnergy: number; profitPerTap: number }; items: InventoryEntry[] }> { return (await httpClient.post('/api/inventory/activate', { itemId })).data as { state: { coins: number; energy: number; maximumEnergy: number; profitPerTap: number }; items: InventoryEntry[] }; }
