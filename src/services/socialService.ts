...
export interface ProfileData { state: { coins: number; xp: number; level: number; rank: string; profitPerTap: number; profitPerHour: number }; inventory: unknown[]; referral: { invited: number; earned: number }; payments: unknown[]; }
