...
export class PostgresAdminPersistence implements AdminPersistence {
  ...
  async setBanned(adminId: string, userId: string, banned: boolean, now: number): Promise<void> {
    if (banned) await this.database.query('INSERT INTO mtx_admin_bans (user_id, admin_id, created_at) VALUES ($1, $2, $3) ON CONFLICT (user_id) DO UPDATE SET admin_id = EXCLUDED.admin_id, created_at = EXCLUDED.created_at', [userId, adminId, new Date(now).toISOString()]);
    else await this.database.query('DELETE FROM mtx_admin_bans WHERE user_id = $1', [userId]);
    await this.log(adminId, banned ? 'user.ban' : 'user.unban', userId, now);
  }
  async notify(adminId: string, item: AdminNotification): Promise<void> { await this.database.query('INSERT INTO mtx_notifications (id, title, message, created_at) VALUES ($1, $2, $3, $4)', [item.id, item.title, item.message, new Date(item.createdAt).toISOString()]); await this.log(adminId, 'notification.create', item.id, item.createdAt); }
  async createEvent(adminId: string, item: AdminEvent): Promise<void> { await this.database.query('INSERT INTO mtx_events (id, title, starts_at, ends_at, multiplier) VALUES ($1, $2, $3, $4, $5)', [item.id, item.title, new Date(item.startsAt).toISOString(), new Date(item.endsAt).toISOString(), item.multiplier]); await this.log(adminId, 'event.create', item.id, Date.now()); }
  ...
  private async log(adminId: string, action: string, target: string, now: number): Promise<void> { await this.database.query('INSERT INTO mtx_admin_logs (id, admin_id, action, target, created_at) VALUES ($1, $2, $3, $4, $5)', [crypto.randomUUID(), adminId, action, target, new Date(now).toISOString()]); }
}
