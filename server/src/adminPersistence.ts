...
interface NotificationRow { id: string; title: string; message: string; created_at: string | Date; expires_at: string | Date; }
...
export class PostgresAdminPersistence implements AdminPersistence {
  ...
  async notify(adminId: string, item: AdminNotification): Promise<void> { await this.inTransaction(async (database) => { await database.query('INSERT INTO mtx_notifications (id, title, message, created_at, expires_at) VALUES ($1, $2, $3, $4, $5)', [item.id, item.title, item.message, new Date(item.createdAt).toISOString(), new Date(item.expiresAt).toISOString()]); await this.log(database, adminId, 'notification.create', item.id, item.createdAt); }); }
  ...
  async snapshot(): Promise<AdminSnapshot> {
    const [bans, logs, notifications, events] = await Promise.all([
      ...
      this.database.query<NotificationRow>('SELECT id, title, message, created_at, expires_at FROM mtx_notifications ORDER BY created_at DESC LIMIT 1000', []),
    ...
    return {
      ...
      notifications: notifications.rows.map((row): AdminNotification => ({ id: row.id, title: row.title, message: row.message, createdAt: time(row.created_at), expiresAt: time(row.expires_at) })),
