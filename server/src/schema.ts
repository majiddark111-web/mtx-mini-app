...
export const referralAcceptSchema: Schema<{ code: string; deviceHash: string }> = { parse(value) { const body = value as Record<string, unknown>; if (!body || typeof body !== 'object' || Object.keys(body).some((key) => key !== 'code' && key !== 'deviceHash') || typeof body.code !== 'string' || !/^MTX-[a-zA-Z0-9_-]{1,64}$/.test(body.code) || typeof body.deviceHash !== 'string' || !/^[a-f0-9]{64}$/.test(body.deviceHash)) throw new ValidationError('Invalid referral'); return { code: body.code, deviceHash: body.deviceHash }; } };
...
export const emptyBodySchema: Schema<Record<string, never>> = { parse(value) { if (!value || typeof value !== 'object' || Array.isArray(value) || Object.keys(value).length) throw new ValidationError('Body must be an empty object'); return {}; } };
