...
import { activityFor, activityPeriodKey, type ActivityTotals } from './periodActivity.ts';
...
export interface LeaderboardRepository { record(userId: string, username: string, coins: number, now?: number, activity?: ActivityTotals): Promise<void>; leaders(limit: number, scope?: LeaderboardScope, now?: number): Promise<LeaderboardEntry[]>; }
export const leaderboardPeriodKey = (scope: LeaderboardScope, now: number): string => scope === 'global' ? 'global' : activityPeriodKey(scope, now);
export class MemoryLeaderboardRepository implements LeaderboardRepository {
  ...
  async record(userId: string, username: string, coins: number, now = Date.now(), activity?: ActivityTotals): Promise<void> {
    for (const scope of ['global', 'weekly', 'monthly', 'season'] as const) {
      const score = scope === 'global' ? coins : activityFor({ activity }, scope, now).earnedCoins;
      if (scope !== 'global' && score <= 0) continue;
      const key = `${scope}:${leaderboardPeriodKey(scope, now)}`;
      const board = this.scores.get(key) ?? new Map();
      const previous = board.get(userId);
      board.set(userId, { username: username === userId ? previous?.username ?? username : username, coins: scope === 'global' ? score : Math.max(previous?.coins ?? 0, score) });
      this.scores.set(key, board);
    }
  }
...
const missionPeriodKey = (now: number, period: MissionView['period']): string => activityPeriodKey(period, now);
...
export class SocialStorage {
  ...
  async claimMission(userId: string, missionId: string, game: GameStorage, now: number): Promise<{ reward: number; state: ServerGameState }> {
    ...
    game.saveHot(updated, !this.persistence); await this.recordScore(userId, userId, updated.coins, now, updated.activity);
  ...
  async claimDaily(userId: string, game: GameStorage, now: number): Promise<{ reward: number; streak: number; state: ServerGameState }> {
    ...
    if (this.persistence) { const saved = await this.persistence.claimDaily(userId, dayKey(now), yesterday, state); if (saved === null) throw new Error('DAILY_ALREADY_CLAIMED'); streak = saved.streak; updated = saved.state; } else { this.dailyClaims.set(userId, { day: dayKey(now), streak }); const reward = streak * 100; updated = { ...state, coins: state.coins + reward, version: state.version + 1 }; } const reward = streak * 100; game.saveHot(updated, !this.persistence); await this.recordScore(userId, userId, updated.coins, now, updated.activity);
  ...
  recordScore(userId: string, username: string, coins: number, now = Date.now(), activity?: ActivityTotals): Promise<void> { return this.leaderboard.record(userId, username, coins, now, activity); }
