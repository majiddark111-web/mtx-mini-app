...
export class SocialStorage {
  ...
  missions(userId: string, state: ServerGameState, now: number): MissionView[] {
    const definitions = [
    ...
    return definitions.map((mission) => ({ ...mission, progress: Math.min(mission.target, mission.progress), claimed: this.missionClaims.has(`${userId}:${mission.id}:${dayKey(now)}`) }));
  }
  ...
  async claimMission(userId: string, missionId: string, game: GameStorage, now: number): Promise<{ reward: number; state: ServerGameState }> {
    ...
    const mission = this.missions(userId, state, now).find((item) => item.id === missionId);
    if (!mission || mission.claimed || mission.progress < mission.target) throw new Error('MISSION_UNAVAILABLE');
    this.missionClaims.add(`${userId}:${mission.id}:${dayKey(now)}`);
  ...
  dailyStatus(userId: string, now: number): { streak: number; reward: number; claimed: boolean } {
    const claim = this.dailyClaims.get(userId); const claimed = claim?.day === dayKey(now); const streak = claim?.streak ?? 0;
  ...
  async claimDaily(userId: string, game: GameStorage, now: number): Promise<{ reward: number; streak: number; state: ServerGameState }> {
    const status = this.dailyStatus(userId, now); if (status.claimed) throw new Error('DAILY_ALREADY_CLAIMED');
    const previous = this.dailyClaims.get(userId); const yesterday = dayKey(now - 86_400_000); const streak = previous?.day === yesterday ? Math.min(7, previous.streak + 1) : 1; const reward = streak * 100;
    this.dailyClaims.set(userId, { day: dayKey(now), streak }); const state = await game.stateFor(userId, now); const updated = { ...state, coins: state.coins + reward, version: state.version + 1 }; game.saveHot(updated); await this.recordScore(userId, userId, updated.coins);
  ...
  async acceptReferral(refereeId: string, code: string, deviceHash: string, game: GameStorage, now: number): Promise<void> {
    ...
    this.referrals.set(refereeId, { referrerId, refereeId, deviceHash, createdAt: now }); this.referralDevices.add(deviceHash);
