...
export class PostgresSocialPersistence implements SocialPersistence {
  ...
  async createReferral(referrerId: string, refereeId: string, deviceHash: string, now: number): Promise<'created' | 'referee-used' | 'device-used'> { const referee = await this.database.query<{ referee_id: string }>('SELECT referee_id FROM mtx_referrals WHERE referee_id = $1', [refereeId]); if (referee.rows.length) return 'referee-used'; const device = await this.database.query<{ referee_id: string }>('SELECT referee_id FROM mtx_referrals WHERE device_hash = $1', [deviceHash]); if (device.rows.length) return 'device-used'; const result = await this.database.query<{ referee_id: string }>('INSERT INTO mtx_referrals (referee_id, referrer_id, device_hash, created_at) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING RETURNING referee_id', [refereeId, referrerId, deviceHash, new Date(now).toISOString()]); return result.rows.length ? 'created' : 'referee-used'; }
