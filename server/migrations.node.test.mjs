...
describe('database migrations', () => {
  it('applies each version once and records its checksum', async () => {
    ...
    assert.deepEqual(await runMigrations(database), ['001_initial.sql', '002_equipped_cosmetics.sql', '003_notification_read_state.sql', '004_tap_receipts.sql']);
    assert.deepEqual(await runMigrations(database), []);
    assert.equal(schemaExecutions, 1);
    assert.equal(applied.size, 4);
  });
});
